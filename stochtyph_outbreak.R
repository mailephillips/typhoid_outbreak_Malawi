...
stochtyph_identify = function(tmax=4173, dt=.1, vax=FALSE, catchup=FALSE, threshIR=1000, threshSD=1000, t_to_vax=NA){
  ...
  births = deaths_S1 = deaths_I1 = deaths_R = deaths_S2 = deaths_I2 = deaths_C = deaths_V1 = deaths_V2 = 
    ...
    NVaxrout = NVaxcamp = v = massvacc = FirstInfNew = matrix(0, nrow=tf, ncol=al)
  
   ### initial number of people
  S1[1,] = round(sapply(sapply(c(.9*N0[1:7],.89*N0[8:al]), FUN=function(x) x-17), FUN=function(x) x*dt))
  I1[1,] = round(sapply(rep(17,al), FUN=function(x) x*dt))
  R[1,]  = rep(0,al)*dt
  ...
  while ((t<=tf)){
    ...
    if(t %% (4/dt) == 0) {
      ...
      new.SD.mth      <- (mean(D.mth,na.rm = T) + (threshSD*(sd(D.mth,na.rm = T))))
      if ((t>(2400/dt)) ) {
    ...
    #if outbreak has been detected:
    if (outbreak == 1 & vax == TRUE) {
      vax_t <- vax_t + 1 #track timesteps until vax starts after outbreak
      if (vax_t >= t_to_vax){ #if it has been the duration of delay time, start vaccination
        v[t,agevacc]  <- vcov_R.t[vax_t]
        if (catchup==TRUE) {
          catchup_camp          <- catchup_camp +1
          if (catchup_camp < (4/dt)) { #if using catchup campaign, do it for 4 timesteps 
            massvacc[t,agevacc:4] <- min((1-(1-vcov_C)^.25),.625)
          }
        }
      } 
    } 
  ...
  for (i in 1:tmax){
    for (j in 1:al){
      FirstInf[i,j] = sum(FirstInfNew[((i-1)*(1/dt)+1):(i/dt),j])
    }
    FirstInfNorm[i,] = FirstInf[i,]/sum(FirstInf[i,])
  }
  
  FirstInfNorm[which(is.na(FirstInfNorm))] <- 0
